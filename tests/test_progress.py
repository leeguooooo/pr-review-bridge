...
class ProgressTests(unittest.TestCase):
 ...
 def test_same_comment_is_edited_and_unchanged_body_is_not_reposted(self):
  ...
  with patch('pr_review.service.REPO','example/project'):
   marker='<!-- pr-review-status:example/project:40 -->'
   g.comments=lambda n:[{'id':9,'user':{'username':'robot'},'body':'queued\n\n'+marker}]
   self.assertEqual(g.status_comment(40,'queued'),'https://git/example/project/pulls/40#issuecomment-9')
 ...
 def test_human_cannot_spoof_status_comment(self):
  ...
  with patch('pr_review.service.REPO','example/project'):
   g.comments=lambda n:[{'id':9,'user':{'username':'human'},'body':'<!-- pr-review-status:example/project:40 -->'}]
 ...
 def test_new_revision_does_not_reuse_completed_old_status(self):
  with tempfile.TemporaryDirectory() as d,patch('pr_review.service.REPO','example/project'):
