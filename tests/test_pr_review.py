...
class ReviewTests(unittest.TestCase):
 def setUp(self):
  self.repo_patch=patch("pr_review.service.REPO","example/project");self.repo_patch.start();self.addCleanup(self.repo_patch.stop)
 ...
 def test_events_allowlist(self):
  p={'repository':{'full_name':'example/project'},'number':31,'action':'synchronized'}
 ...
 def test_parse_branches_and_reject_foreign(self):
  h='<div class="column title"><div class="label"><i class="octicon-issue-opened"></i>Open</div></div><h1>Closed bug fix</h1><span class="pull-desc"><code>example/feat/x</code><code>example/test</code></span>'
  p=parse_pr(h,'https://git.example',31);self.assertEqual((p['head'],p['base']),('feat/x','test'));self.assertFalse(p['closed'])
  with self.assertRaises(ValueError):parse_pr(h.replace('example/feat','fork/feat'),'https://git.example',31)
 ...
 def test_real_http_signature_and_duplicate_delivery(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   try:
    ...
    body=json.dumps({'repository':{'full_name':'example/project'},'action':'opened','number':31}).encode()
 ...
 def test_robot_api_comment_does_not_reuse_human_marker(self):
  ...
  self.assertEqual(url,'https://git.example/example/project/pulls/31#issuecomment-2')
 ...
 def test_signed_webhook_metadata_supplies_missing_gogs_pr_api_fields(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   save_pr_metadata({'repository':{'full_name':'example/project'},'pull_request':{'number':31,'head_branch':'feat/x','base_branch':'test','head_repo':{'full_name':'example/project'},'base_repo':{'full_name':'example/project'}}},st)
