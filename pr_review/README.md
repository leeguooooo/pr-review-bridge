...
Live integration validation: one isolated PR completed via a pinned Chrome profile using chatgpt-use 0.0.2 and Instant. The structured finding identified the intentional empty-list division-by-zero defect and was published to Gogs. This does not verify every model selection or remote cancellation after process death. Browser stage time budgets are 180 seconds for the light level and 240 seconds for deep; the 300-second parent watchdog remains. `browser_bin_dir` can provide an operator-owned chrome-use wrapper that always pins a browser identity.
